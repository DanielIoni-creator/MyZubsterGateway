...
async function createEscrow({ jobId, clientId, robotId, amount, currency }) {
  ...
  let lockTx;
  if (currency === 'MYZ') {
    lockTx = await lockMYZ(clientId, amount);
  } else if (currency === 'XMR') {
    lockTx = await lockXMR(clientId, amount);
  }
  ...
  const escrow = {
    ...
    lockTx,
    createdAt: Date.now(),
    deadline: Date.now() + JOB_TIMEOUT_HOURS * 3600 * 1000
  };
...
async function markDelivered({ jobId }) {
  ...
  await notifyUser(escrow.clientId, `Robot ha consegnato job ${jobId}. Hai 48h per contestare o si sblocca auto.`);
