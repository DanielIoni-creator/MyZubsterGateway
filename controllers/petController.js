...
exports.getPetByNfc = async (req, res) => {
  try {
    ...
    console.log('🔍 NFC lookup per:', nfcId);
    const pet = await Pet.findOne({ nfcId });
    if (!pet) {
      return res.status(404).json({ success: false, message: 'Pet not found with this NFC ID' });
    }
...
exports.registerPet = async (req, res) => {
  try {
    ...
    if (!name || !species || !moneroAddress) {
      return res.status(400).json({ success: false, message: 'Name, species, and Monero address are required' });
    }
    const pet = new Pet({ name, species, breed, age, weight, nfcId, gps, photos: photos || [], owner: req.user.id, moneroAddress, status: 'pending' });
    await pet.save();
    res.status(201).json({ success: true, message: 'Pet registered successfully', pet });
  } catch (error) {
...
exports.getPetById = async (req, res) => {
  try {
    ...
    if (pet.owner.toString() !== req.user.id) return res.status(403).json({ success: false, message: 'Access denied' });
    res.json({ success: true, pet });
  } catch (error) {
