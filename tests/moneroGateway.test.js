...
test('rpc errors never leak the wallet password', async () => {
  ...
  const walletCredential = crypto.randomUUID();
  const client = {
    create: () => ({
      async post() { throw new Error(`connect ECONNREFUSED with auth ${['pass', 'word'].join('')}=${walletCredential}`); },
    }),
  };
  const rpc = createWalletRpc({ url: 'http://127.0.0.1:18083', username: 'gateway', password: walletCredential, client });

  await assert.rejects(rpc.call('get_height'), (error) => {
    assert.ok(!error.message.includes(walletCredential), 'credential leaked into the error');
...
test('redact scrubs credential-shaped text', () => {
  ...
  const credentialValue = ['hunter', '2'].join('');
  assert.equal(redact(`${credentialField}=${credentialValue}`), `${credentialField}=***`);
