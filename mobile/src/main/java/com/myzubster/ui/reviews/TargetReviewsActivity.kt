...
class TargetReviewsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_target_reviews)

        targetId = intent.getStringExtra("targetId") ?: ""
        targetName = intent.getStringExtra("targetName") ?: "Utente"
    ...
    private fun setupToolbar() {
        ...
        supportActionBar?.title = "Recensioni Ricevute"
    ...
    private fun loadReviews() {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val response = RetrofitClient.apiService.getTargetReviews(
                    targetId = targetId,
                    token = "Bearer $token",
                    page = 1,
                    limit = 20
